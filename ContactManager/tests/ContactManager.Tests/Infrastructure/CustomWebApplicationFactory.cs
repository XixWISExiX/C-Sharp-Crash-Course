...
public sealed class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public CustomWebApplicationFactory(string grpcUrl)
    {
        _grpcUrl = grpcUrl;
    }
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            // OPTIONAL (keep it empty for now):
            // If your API no longer uses its own DbContext because it calls gRPC for everything,
            // you can remove the old DbContext override code from tests entirely.
        });
    }
}
